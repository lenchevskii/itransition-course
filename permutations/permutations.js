...
const repeatables     = (arr)   => arr.map((x) => arr.map((elem, index) => elem === x ? index : undefined)
                                              .filter(z => z != undefined))
                                            .filter(list => list.length > 1)

const getMaxOf2dArray = (arr) => arr.reduce((max, p) => p.length > max.length ? p : max)

const getElemsByArray = (arr) => arr.map(x => args[x])

const prettyPrint     = (arr) => arr.map(x => x != undefined ? console.log(x) : undefined)
